...
function ContextAwareToggle({ eventKey, callback, setActiveKey, title }) {
...
const AccElement = ({ subtitle, description, title, index }) => {

  const localStorageKey = `acсordionState-${index}`;
  ...
  useEffect(() => {
    localStorage.setItem(localStorageKey, JSON.stringify(activeKey));
  }, [activeKey, localStorageKey]);

  return (
    <Accordion className="mb-5" alwaysOpen activeKey={activeKey}>
      <Card className="py-3 border-0 rounded-5 background-color">
        ...
        <Accordion.Collapse eventKey={index.toString()} className="rounded-5">
